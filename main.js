...
TRY("pricing_nube_json",()=>{const Q=(s,c=D)=>c.querySelector(s),money=n=>"$"+Number(n||0).toLocaleString("es-MX"),txt=v=>v===undefined||v===null?"—":String(v),isNube=()=>D.body&&D.body.getAttribute("data-calc")==="nube",PR=()=>W.preciosContpaqi||null,recommendedBySystem={"CONTPAQi Evalúa":"Equipos","CONTPAQi Contabiliza":"Equipos","CONTPAQi Personia":"Equipos","CONTPAQi Analiza":"Equipos","CONTPAQi Optimiza":"Equipos","CONTPAQi Anticipa":"Equipos","CONTPAQi Vende":"Equipos"},hintsBySystem={"CONTPAQi Evalúa":"CONTPAQi Evalúa se licencia por plan anual, con empleados incluidos por plan.","CONTPAQi Contabiliza":"CONTPAQi Contabiliza se maneja por suscripción anual basada en planes.","CONTPAQi Personia":"CONTPAQi Personia se licencia por plan anual con empleados incluidos por plan.","CONTPAQi Analiza":"CONTPAQi Analiza se licencia por plan. Incluye carga de XML ilimitada desde Inicial.","CONTPAQi Optimiza":"CONTPAQi Optimiza se licencia por plan anual según usuarios, RFC y almacenamiento.","CONTPAQi Anticipa":"CONTPAQi Anticipa se licencia por plan anual según usuarios y RFC incluidos.","CONTPAQi Vende":"CONTPAQi Vende se maneja por suscripción anual basada en el plan y número de usuarios."},descBySystem={"CONTPAQi Evalúa":{Inicial:"Ideal para comenzar evaluación laboral",Equipos:"Para áreas o equipos medianos",Empresarial:"Para operación más amplia",Corporativo:"Máxima capacidad y cobertura"},"CONTPAQi Contabiliza":{Inicial:"Ideal para operación básica",Equipos:"Más capacidad para crecer",Empresarial:"Para empresas medianas",Corporativo:"Administración avanzada"},"CONTPAQi Personia":{Inicial:"Ideal para pequeñas empresas",Equipos:"Más capacidad para crecer",Empresarial:"Para empresas medianas",Corporativo:"Administración avanzada"},"CONTPAQi Analiza":{Trial:"Prueba 20 días",Inicial:"Para iniciar BI con CONTPAQi",Equipos:"Colaboración y más RFC",Empresarial:"Para operación multiárea",Corporativo:"Máxima capacidad"},"CONTPAQi Optimiza":{Demo:"Prueba 20 días",Inicial:"Control esencial de flujo",Equipos:"Visión estratégica PYME",Empresarial:"Mayor capacidad operativa",Corporativo:"Poder total e ilimitado"},"CONTPAQi Anticipa":{Demo:"Prueba 20 días",Inicial:"Protección inicial",Equipos:"Más RFC y colaboración",Empresarial:"Para operación multiárea",Corporativo:"Cobertura avanzada"},"CONTPAQi Vende":{LIGHT:"Entrada / operaciones básicas",Inicial:"Para facturar y cobrar sin complicarte",Equipos:"Para operar con 2 usuarios",Empresarial:"Operación multiusuario",Corporativo:"Máxima capacidad"}},recBadge=(sys,plan)=>recommendedBySystem[sys]===plan?`<span class="plan-rec">Recomendado</span>`:"",planLabel=(sys,plan)=>`<span class="plan-name">${plan}${recBadge(sys,plan)}${descBySystem[sys]&&descBySystem[sys][plan]?`<div class="plan-desc">${descBySystem[sys][plan]}</div>`:""}</span>`,row=(cells,cls="")=>`<tr${cls?` class="${cls}"`:""}>${cells.map(x=>`<td>${x}</td>`).join("")}</tr>`,rowCol=(first,middleText,middleSpan,last,cls="row-soft")=>`<tr class="${cls}"><td>${first}</td><td colspan="${middleSpan}">${middleText}</td><td>${last}</td></tr>`,noteRow=(colspan,html)=>`<tfoot><tr><td class="note" colspan="${colspan}">${html}</td></tr></tfoot>`,wrapTable=(thead,tbody,tfoot="")=>`<div class="pricing-table-nube-wrap"><table class="pricing-table-nube" aria-describedby="calc">${thead}<tbody>${tbody}</tbody>${tfoot}</table></div>`,builders={"CONTPAQi Evalúa":p=>{const n=p.nube||{},o=["Inicial","Equipos","Empresarial","Corporativo"];let b="";o.forEach(k=>{const x=n[k];if(!x)return;b+=row([planLabel("CONTPAQi Evalúa",k),txt(x.usuarios_incluidos==="multi"?"Multiusuario":x.usuarios_incluidos),`${txt(x.empleados_incluidos)} empleados`,`<span class="price">${money(x.precio_base)}</span>`],k==="Equipos"?"is-rec":k==="Trial"?"row-soft":"")});const e=o.map(k=>n[k]&&n[k].empleado_adicional?`${k}: ${money(n[k].empleado_adicional)}`:"").filter(Boolean).join(" · ");return wrapTable(`<thead><tr><th>Plan</th><th>Usuarios incluidos</th><th>Empleados incluidos</th><th style="text-align:right">Precio Base Anual</th></tr></thead>`,b,noteRow(4,`<strong>Empleado adicional:</strong> ${e}<br><span class="muted">Precios en MXN + IVA. Licenciamiento anual. Incluye soporte técnico Expiriti.</span>`))},"CONTPAQi Contabiliza":p=>{const n=p.nube||{},o=["Inicial","Equipos","Empresarial","Corporativo"];let b="";o.forEach(k=>{const x=n[k];if(!x)return;b+=row([planLabel("CONTPAQi Contabiliza",k),`${txt(x.usuarios_incluidos)} Usuario${Number(x.usuarios_incluidos)===1?"":"s"}`,`<span class="price">${money(x.precio_base)}</span>`],k==="Equipos"?"is-rec":k==="Trial"?"row-soft":"")});return wrapTable(`<thead><tr><th>Plan</th><th>Usuarios Incluidos</th><th style="text-align:right">Precio Base Anual</th></tr></thead>`,b,noteRow(3,`<strong>Usuario adicional:</strong> ${money(n.usuario_adicional)} anual<br><span class="muted">Precios en MXN + IVA. Licenciamiento anual (365 días).</span>`))},"CONTPAQi Personia":p=>{const n=p.nube||{},o=["Inicial","Equipos","Empresarial","Corporativo"];let b="";o.forEach(k=>{const x=n[k];if(!x)return;b+=row([planLabel("CONTPAQi Personia",k),txt(x.usuarios_incluidos==="multi"?"Multiusuario":x.usuarios_incluidos),`${txt(x.empleados_incluidos)} empleados`,`<span class="price">${money(x.precio_base)}</span>`],k==="Equipos"?"is-rec":k==="Trial"?"row-soft":"")});const e=o.map(k=>n[k]&&n[k].empleado_adicional?`${k}: ${money(n[k].empleado_adicional)}`:"").filter(Boolean).join(" · ");return wrapTable(`<thead><tr><th>Plan</th><th>Usuarios Incluidos</th><th>Empleados Incluidos</th><th style="text-align:right">Precio Base Anual</th></tr></thead>`,b,noteRow(4,`<strong>Empleado adicional:</strong> ${e}<br><span class="muted">Precios en MXN + IVA. Licenciamiento anual (365 días). Incluye soporte técnico Expiriti.</span>`))},"CONTPAQi Analiza":p=>{const n=p.nube||{},o=["Trial","Inicial","Equipos","Empresarial","Corporativo"];let b="";o.forEach(k=>{const x=n[k];if(!x)return;b+=row([planLabel("CONTPAQi Analiza",k),txt(x.usuarios_incluidos),txt(x.rfc_incluidos),txt(x.carga_xml==="ilimitado"?"Ilimitado":x.carga_xml),`<span class="price">${money(x.precio_base)}</span>`],k==="Equipos"?"is-rec":k==="Trial"?"row-soft":"")});if(n.usuario_adicional)b+=rowCol(`<span class="plan-name">Usuario adicional</span>`,"Añade 1 usuario",3,`<span class="price">${money(n.usuario_adicional)}</span>`);if(n.rfc_adicional&&typeof n.rfc_adicional==="object"){["Inicial","Equipos","Empresarial","Corporativo"].forEach(k=>{if(!n.rfc_adicional[k])return;b+=rowCol(`<span class="plan-name">RFC adicional (${k})</span>`,"Añade 1 RFC",3,`<span class="price">${money(n.rfc_adicional[k])}</span>`);})}return wrapTable(`<thead><tr><th>Plan</th><th>Usuarios</th><th>RFC incluidos</th><th>Carga XML</th><th style="text-align:right">Precio Base Anual</th></tr></thead>`,b,noteRow(5,`Precios en MXN + IVA. Licenciamiento anual (${txt(n.licenciamiento_dias||365)} días). Incluye soporte técnico Expiriti.`))},"CONTPAQi Optimiza":p=>{const n=p.nube||{},o=["Demo","Inicial","Equipos","Empresarial","Corporativo"];let b="";o.forEach(k=>{const x=n[k];if(!x)return;b+=row([planLabel("CONTPAQi Optimiza",k),txt(x.usuarios_incluidos),txt(x.rfc_incluidos),`${txt(x.almacenamiento_gb)} GB`,`<span class="price">${money(x.precio_base)}</span>`],k==="Equipos"?"is-rec":k==="Demo"?"row-soft":"")});if(n.usuario_adicional)b+=rowCol(`<span class="plan-name">Usuario adicional</span>`,"Añade 1 usuario",3,`<span class="price">${money(n.usuario_adicional)}</span>`);if(n.rfc_adicional)b+=rowCol(`<span class="plan-name">RFC adicional</span>`,"Añade 1 RFC",3,`<span class="price">${money(n.rfc_adicional)}</span>`);if(n.espacio_adicional&&typeof n.espacio_adicional==="object"){Object.keys(n.espacio_adicional).forEach(k=>{b+=rowCol(`<span class="plan-name">Espacio adicional (${k})</span>`,"Almacenamiento extra",3,`<span class="price">${money(n.espacio_adicional[k])}</span>`);})}return wrapTable(`<thead><tr><th>Plan</th><th>Usuarios</th><th>RFC incluidos</th><th>Almacenamiento</th><th style="text-align:right">Precio Base Anual</th></tr></thead>`,b,noteRow(5,`Precios en MXN + IVA. Licenciamiento anual. Incluye soporte técnico Expiriti.`))},"CONTPAQi Anticipa":p=>{const n=p.nube||{},o=["Demo","Inicial","Equipos","Empresarial","Corporativo"];let b="";o.forEach(k=>{const x=n[k];if(!x)return;b+=row([planLabel("CONTPAQi Anticipa",k),txt(x.usuarios_incluidos),txt(x.rfc_incluidos),`<span class="price">${money(x.precio_base)}</span>`],k==="Equipos"?"is-rec":k==="Demo"?"row-soft":"")});if(n.usuario_adicional)b+=rowCol(`<span class="plan-name">Usuario adicional</span>`,"Añade 1 usuario",2,`<span class="price">${money(n.usuario_adicional)}</span>`);if(n.rfc_adicional&&typeof n.rfc_adicional==="object"){["Inicial","Equipos","Empresarial","Corporativo"].forEach(k=>{if(!n.rfc_adicional[k])return;b+=rowCol(`<span class="plan-name">RFC adicional (${k})</span>`,"Añade 1 RFC",2,`<span class="price">${money(n.rfc_adicional[k])}</span>`);})}return wrapTable(`<thead><tr><th>Plan</th><th>Usuarios</th><th>RFC incluidos</th><th style="text-align:right">Precio Base Anual</th></tr></thead>`,b,noteRow(4,`Precios en MXN + IVA. Licenciamiento anual. Incluye soporte técnico Expiriti.`))},"CONTPAQi Vende":p=>{const n=p.nube||{},o=["LIGHT","Inicial","Equipos","Empresarial","Corporativo"];let b="";o.forEach(k=>{const x=n[k];if(!x)return;const m={LIGHT:"Funciones esenciales del sistema",Inicial:"Para la mayoría de PyMEs",Equipos:"Mejor para equipos",Empresarial:"Para equipos con más volumen",Corporativo:"Operación avanzada"};b+=row([planLabel("CONTPAQi Vende",k),`${txt(x.usuarios_incluidos)} Usuario${Number(x.usuarios_incluidos)===1?"":"s"}`,m[k]||"—",`<span class="price">${money(x.precio_base)}</span>`],k==="Equipos"?"is-rec":"")});if(n.usuario_adicional)b+=row([`<span class="plan-name">Usuario adicional</span>`,"Añade 1 usuario","Licenciamiento anual",`<span class="price">${money(n.usuario_adicional)}</span>`],"row-soft");return wrapTable(`<thead><tr><th>Plan</th><th>Usuarios Incluidos</th><th>Notas</th><th style="text-align:right">Precio Base Anual</th></tr></thead>`,b,noteRow(4,`Precios en MXN + IVA. Licenciamiento anual (365 días). Incluye soporte técnico Expiriti.`))}},render=()=>{if(!isNube())return;const a=Q("#app"),s=Q("#calculadora-section"),d=PR();if(!a||!s||!d)return;const sys=(a.dataset.system||"").trim();if(!sys||sys==="CONTPAQi Despachos")return;const product=d[sys],build=builders[sys];if(!product||!product.nube||typeof build!=="function")return;if(hintsBySystem[sys]){const h=Q(".hint",s);h&&(h.textContent=hintsBySystem[sys])}const card=Q(".card.body",s);if(!card)return;const html=build(product);if(!html)return;const oldWrap=Q(".pricing-table-nube-wrap",card),oldTable=Q(".pricing-table-nube",card);if(oldWrap)oldWrap.outerHTML=html;else if(oldTable)oldTable.outerHTML=html;else{const hint=Q(".hint",card);hint?hint.insertAdjacentHTML("beforebegin",html):card.insertAdjacentHTML("afterbegin",html)}},boot=()=>render();D.readyState==="loading"?D.addEventListener("DOMContentLoaded",boot,{once:!0}):boot();W.addEventListener("pageshow",boot,{passive:!0})});
TRY("pricing_nube_refine",()=>{const mark=()=>{D.querySelectorAll(".pricing-table-nube").forEach(t=>{const rows=[...t.querySelectorAll("tbody tr")];let cut=!1;rows.forEach(r=>{r.classList.remove("first-extra");if(r.classList.contains("row-soft")&&!cut){r.classList.add("first-extra");cut=!0}})})};D.readyState==="loading"?D.addEventListener("DOMContentLoaded",mark,{once:!0}):mark();W.addEventListener("pageshow",mark,{passive:!0})});

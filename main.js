...
/* EXPIRITI — CALC UI (HOOKS+HEADERS+ICONS) v2026.02.04 (MIN | SAFE | TERSER OK) */
TRY("calc_ui_safe",()=>{const D=document,W=window,$=id=>D.getElementById(id);const has=()=>{const a=$("app");return!!(a&&((a.dataset.system||"").trim())&&$("calc-row"))},fire=()=>{try{W.dispatchEvent(new Event("calc-recompute"))}catch(e){}};D.readyState==="loading"?D.addEventListener("DOMContentLoaded",()=>{has()&&fire()},{once:!0}):has()&&fire();W.addEventListener("pageshow",()=>{has()&&fire()},{passive:!0});
/* HEADERS + NOTES + CONTPAQi® */
(()=>{let lock=0,raf=0;const txt=id=>{const e=$(id);return e?(e.textContent||"").trim():""},setT=(el,v)=>{v=v||"";el&&el.textContent!==v&&(el.textContent=v)},setA=(el,a,v)=>{v=v||"";el&&el.getAttribute(a)!==v&&el.setAttribute(a,v)},logoOf=n=>{const c=W.CATALOG_SISTEMAS||[];for(let i=0;i<c.length;i++)if(c[i]&&c[i].name===n)return c[i].imgLogo||"";return""},head=el=>{if(!el)return null;let h=el.querySelector(".calc-head");if(h)return h;h=D.createElement("div");h.className="calc-head";h.innerHTML='<div class="calc-head-row"><img class="calc-syslogo" alt="" decoding="async"><div class="calc-head-text"><div class="calc-price"></div><div class="calc-sub"></div></div></div>';el.insertBefore(h,el.firstChild);return h},brandRG=root=>{if(!root)return;const nodes=root.querySelectorAll(".calc-sub,.sys-icon strong");for(let i=0;i<nodes.length;i++){const el=nodes[i];if(!el||el.dataset.calcRg==="1")continue;const t=el.textContent||"";if(!/\bCONTPAQi\b/.test(t))continue;el.textContent=/\bCONTPAQi®\b/.test(t)?t:t.replace(/\bCONTPAQi\b/g,"CONTPAQi\u00AE");el.dataset.calcRg="1"}},setCalcHead=(el,name,price)=>{const h=head(el);if(!h)return;const img=h.querySelector(".calc-syslogo"),p=h.querySelector(".calc-price"),s=h.querySelector(".calc-sub"),src=logoOf(name);src?(img.style.display!==""&&(img.style.display=""),setA(img,"src",src),setA(img,"alt",name||"")):img.style.display!=="none"&&(img.style.display="none");setT(p,price||"");setT(s,name||"");brandRG(h)},ensureNote=(slot,title,sub)=>{if(!slot)return;let note=slot.querySelector(".note");note||(note=D.createElement("div"),note.className="note",slot.prepend(note));let st=note.querySelector("strong");st||(st=D.createElement("strong"),note.appendChild(st));let sm=note.querySelector("small");sm||(sm=D.createElement("small"),note.appendChild(sm));setT(st,title||"");sm.innerHTML=sub?("<em>"+sub+"</em>"):"";note.classList.add("note-center")},run=()=>{if(lock)return;lock=1;try{const p=$("calc-primary"),s=$("calc-secondary"),t=$("calc-tertiary"),slot=$("calc-slot-2"),add=$("add-more-panel");p&&setCalcHead(p,p.dataset.systemName||"",txt("tot1"));s&&s.querySelector("table")&&setCalcHead(s,s.dataset.systemName||"",txt("tot2"));t&&t.style.display!=="none"&&t.querySelector("table")&&setCalcHead(t,t.dataset.systemName||"",txt("tot3"));slot&&ensureNote(slot,"Integra otro sistema y recibe −15%","XML en Línea+ no aplica para descuento.");add&&ensureNote(add,"¿Agregar un tercer sistema?","Mismo -15% por paquete, excepto XML en Línea+.");brandRG($("calculadora-section")||D)}catch(e){}finally{lock=0}},runQ=()=>{if(raf)return;raf=requestAnimationFrame(()=>{raf=0;run()})};D.readyState==="loading"?D.addEventListener("DOMContentLoaded",runQ,{once:!0}):runQ();W.addEventListener("calc-recompute",runQ,{passive:!0});W.addEventListener("calc-render",runQ,{passive:!0});W.addEventListener("pageshow",runQ,{passive:!0})})();
/* ICONS CAROUSEL (SAFE | TERSER OK) */
(()=>{const mk=(cls,lab,chev)=>{const b=D.createElement("button");b.type="button";b.className="arrowCircle "+cls;b.setAttribute("aria-label",lab);b.innerHTML='<span class="chev">'+chev+"</span>";return b};const enhance=wrap=>{if(!wrap)return;const ensure=()=>{if(wrap.dataset.icInit==="1")return;if(!wrap.querySelector(".sys-icon"))return;wrap.dataset.icInit="1";let host=wrap.closest(".icons-carousel");host||(host=D.createElement("div"),host.className="icons-carousel",wrap.parentElement.insertBefore(host,wrap),host.appendChild(wrap));let prev=host.querySelector(".arrowCircle.prev"),next=host.querySelector(".arrowCircle.next");prev||(prev=mk("prev","Anterior","‹"),host.appendChild(prev));next||(next=mk("next","Siguiente","›"),host.appendChild(next));const step=()=>{const it=wrap.querySelector(".sys-icon");const w=it&&it.offsetWidth?it.offsetWidth:200;return Math.max(220,Math.round(w+18))};const scroll=dir=>wrap.scrollBy({left:step()*dir,behavior:"smooth"});prev.onclick=()=>scroll(-1);next.onclick=()=>scroll(1);let raf=0;const paint=()=>{raf=0;const can=wrap.scrollWidth>wrap.clientWidth+4;prev.style.display=can?"":"none";next.style.display=can?"":"none";const slot=wrap.closest("#calc-slot-2,.calc-container,.placeholder")||wrap.parentElement;const note=slot&&slot.querySelector?slot.querySelector(".note"):null;note&&note.classList.add("note-center")};const paintQ=()=>{raf||(raf=requestAnimationFrame(paint))};paintQ();if(!wrap.dataset.icResize){wrap.dataset.icResize="1";W.addEventListener("resize",paintQ,{passive:!0})}if(!wrap.dataset.icObs){wrap.dataset.icObs="1";new MutationObserver(paintQ).observe(wrap,{childList:!0})}};ensure();if(!wrap.dataset.icBootObs){wrap.dataset.icBootObs="1";new MutationObserver(ensure).observe(wrap,{childList:!0})}};const boot=()=>{enhance($("icons-sec-sys"));enhance($("icons-third-sys"))};D.readyState==="loading"?D.addEventListener("DOMContentLoaded",boot,{once:!0}):boot();W.addEventListener("calc-recompute",boot,{passive:!0});W.addEventListener("calc-render",boot,{passive:!0});W.addEventListener("pageshow",boot,{passive:!0});})();});

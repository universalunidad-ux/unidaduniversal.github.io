...
(()=>{try{const o=()=>{const t=window.__EXP_ABS__||((t)=>t);document.querySelectorAll(".cta-strip[data-sistema]").forEach((e,s)=>{const r=(e.dataset.sistema||"").trim(),n=(e.dataset.pdf||"").trim(),c=o=>"https://wa.me/5568437918?text="+encodeURIComponent(o),a=n?t(`PDFS/${n}.pdf`):"#",i=t("IMG/tocar.webp"),d=`cta-map-${s+1}`;e.innerHTML=`<div class="cta-map-wrap"><button class="cta-map-toggle" type="button" aria-expanded="false" aria-controls="${d}"><img class="cta-map-icon" src="${i}" alt="" aria-hidden="true"><span>Mapa</span></button><div class="cta-map-menu" id="${d}" hidden><a class="btn btn-grad-green hero-btn" href="${c(`Hola Expiriti, quiero comprar mi licencia de CONTPAQi ${r}`)}" target="_blank" rel="noopener">Compra ahora</a><a class="btn btn-grad-purple hero-btn" href="${c(`Hola Expiriti, quiero mi prueba gratis de 30 días de CONTPAQi ${r}`)}" target="_blank" rel="noopener">Prueba gratis</a><a class="btn btn-grad-blue hero-btn" href="${c(`Hola Expiriti, quiero agendar una demo de 45 min por Zoom de CONTPAQi ${r}`)}" target="_blank" rel="noopener">Demo de 45 min por Zoom</a>${n?`<a class="btn btn-ghost hero-btn cta-ghost-black" href="${a}" target="_blank" rel="noopener">Ficha técnica (PDF)</a>`:""}<a class="btn btn-ghost hero-btn cta-ghost-black" href="#caracteristicas">Resumen ficha técnica</a></div></div>`;const l=e.querySelector(".cta-map-toggle"),m=e.querySelector(".cta-map-menu");if(!l||!m)return;const close=()=>{l.setAttribute("aria-expanded","false");m.hidden=!0;e.classList.remove("is-open")},open=()=>{document.querySelectorAll(".cta-strip.is-open").forEach(x=>{if(x!==e){x.classList.remove("is-open");const b=x.querySelector(".cta-map-toggle"),p=x.querySelector(".cta-map-menu");b&&b.setAttribute("aria-expanded","false");p&&(p.hidden=!0)}});l.setAttribute("aria-expanded","true");m.hidden=!1;e.classList.add("is-open")};l.addEventListener("click",f=>{f.preventDefault();f.stopPropagation();e.classList.contains("is-open")?close():open()});m.querySelectorAll("a").forEach(a=>a.addEventListener("click",()=>close(),{passive:!0}));document.addEventListener("click",f=>{if(!e.contains(f.target))close()});document.addEventListener("keydown",f=>{"Escape"===f.key&&close()})})};"loading"===document.readyState?document.addEventListener("DOMContentLoaded",o,{once:!0}):o()}catch(o){console.warn("[main.js] cta_strip_dynamic falló",o)}})();

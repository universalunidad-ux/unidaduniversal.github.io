...
(()=>{try{const o=()=>{const abs=window.__EXP_ABS__||((t)=>t);document.querySelectorAll(".cta-strip[data-sistema]").forEach((wrap,i)=>{const sys=(wrap.dataset.sistema||"").trim(),pdf=(wrap.dataset.pdf||"").trim(),wa=t=>"https://wa.me/5568437918?text="+encodeURIComponent(t),pdfUrl=pdf?abs(`PDFS/${pdf}.pdf`):"#",icon=abs("IMG/tocar.webp"),id=`cta-map-${i+1}`;wrap.innerHTML=`<div class="cta-map-wrap"><button class="cta-map-toggle" type="button" aria-expanded="false" aria-controls="${id}"><img class="cta-map-icon" src="${icon}" alt="" aria-hidden="true"><span>Mapa</span></button><div class="cta-map-menu" id="${id}" hidden><a class="btn btn-grad-green hero-btn" href="${wa(`Hola Expiriti, quiero comprar mi licencia de CONTPAQi ${sys}`)}" target="_blank" rel="noopener">Compra ahora</a><a class="btn btn-grad-purple hero-btn" href="${wa(`Hola Expiriti, quiero mi prueba gratis de 30 días de CONTPAQi ${sys}`)}" target="_blank" rel="noopener">Prueba gratis</a><a class="btn btn-grad-blue hero-btn" href="${wa(`Hola Expiriti, quiero agendar una demo de 45 min por Zoom de CONTPAQi ${sys}`)}" target="_blank" rel="noopener">Demo de 45 min por Zoom</a>${pdf?`<a class="btn btn-ghost hero-btn cta-ghost-black" href="${pdfUrl}" target="_blank" rel="noopener">Ficha técnica (PDF)</a>`:""}<a class="btn btn-ghost hero-btn cta-ghost-black" href="#caracteristicas">Resumen ficha técnica</a></div></div>`;const btn=wrap.querySelector(".cta-map-toggle"),menu=wrap.querySelector(".cta-map-menu");if(!btn||!menu)return;const close=()=>{btn.setAttribute("aria-expanded","false");menu.hidden=!0;wrap.classList.remove("is-open")},open=()=>{document.querySelectorAll(".cta-strip.is-open").forEach(x=>{if(x!==wrap){x.classList.remove("is-open");const b=x.querySelector(".cta-map-toggle"),m=x.querySelector(".cta-map-menu");b&&b.setAttribute("aria-expanded","false");m&&(m.hidden=!0)}});btn.setAttribute("aria-expanded","true");menu.hidden=!1;wrap.classList.add("is-open")};btn.addEventListener("click",e=>{e.preventDefault();e.stopPropagation();wrap.classList.contains("is-open")?close():open()});menu.querySelectorAll("a").forEach(a=>a.addEventListener("click",()=>close(),{passive:!0}));document.addEventListener("click",e=>{if(!wrap.contains(e.target))close()});document.addEventListener("keydown",e=>{"Escape"===e.key&&close()})})};"loading"===document.readyState?document.addEventListener("DOMContentLoaded",o,{once:!0}):o()}catch(e){console.warn("[main.js] cta_strip_dynamic falló",e)}})();

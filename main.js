...
TRY("toc_actions",()=>{const e=D.getElementById("toc-actions");if(!e)return;const t=e.querySelector(".toc-toggle"),p=e.querySelector(".toc-panel"),c=Array.from(e.querySelectorAll("a")),O="open",R="collapsed",isOpen=()=>e.classList.contains(O)&&!e.classList.contains(R),set=a=>{e.classList.toggle(O,!!a);e.classList.toggle(R,!a);e.setAttribute("aria-hidden",a?"false":"true");t&&t.setAttribute("aria-expanded",a?"true":"false");p&&(p.hidden=!a)},open=()=>set(!0),close=()=>set(!1),toggle=()=>isOpen()?close():open();set(!e.classList.contains(R));t&&t.addEventListener("click",ev=>{ev.preventDefault();ev.stopPropagation();toggle()},{passive:!1});c.forEach(a=>a.addEventListener("click",close,{passive:!0}));D.addEventListener("keydown",ev=>{if(ev.key==="Escape")close()});D.addEventListener("click",ev=>{if(!isOpen())return;const trg=ev.target;if(e.contains(trg))return;if(t&&t.contains(trg))return;close()})});
...
(()=>{try{const o=()=>{const abs=window.__EXP_ABS__||((t)=>t),app=document.getElementById("app");if(!app)return;const sys=(app.dataset.system||"").trim(),pdf=(app.dataset.pdf||""),wa=t=>"https://wa.me/5568437918?text="+encodeURIComponent(t),pdfUrl=pdf?abs(`PDFS/${pdf}.pdf`):"#";if(document.getElementById("toc-actions"))return;const el=document.createElement("aside");el.id="toc-actions";el.className="toc collapsed";el.style.cssText="position:fixed;right:16px;bottom:72px;z-index:9999";el.innerHTML=`<button class="toc-toggle" aria-expanded="false">Más⚡</button><div class="toc-panel" hidden><a href="${wa(`Hola Expiriti, quiero comprar mi licencia de CONTPAQi ${sys}`)}" target="_blank" rel="noopener">Compra ahora</a><a href="${wa(`Hola Expiriti, quiero mi prueba gratis de 30 días de CONTPAQi ${sys}`)}" target="_blank" rel="noopener">Prueba gratis</a><a href="${wa(`Hola Expiriti, quiero agendar una demo de 45 min por Zoom de CONTPAQi ${sys}`)}" target="_blank" rel="noopener">Demo de 45 min por Zoom</a>${pdf?`<a href="${pdfUrl}" target="_blank" rel="noopener">Ficha técnica (PDF)</a>`:""}<a href="#caracteristicas">Resumen ficha técnica</a></div>`;document.body.appendChild(el);const btn=el.querySelector(".toc-toggle"),panel=el.querySelector(".toc-panel");btn.addEventListener("click",e=>{e.preventDefault();const open=panel.hidden;panel.hidden=!open;btn.setAttribute("aria-expanded",open?"true":"false");el.classList.toggle("open",open)});document.addEventListener("click",e=>{if(!el.contains(e.target)){panel.hidden=!0;btn.setAttribute("aria-expanded","false");el.classList.remove("open")}});document.addEventListener("keydown",e=>{"Escape"===e.key&&(panel.hidden=!0,btn.setAttribute("aria-expanded","false"),el.classList.remove("open"))})};"loading"===document.readyState?document.addEventListener("DOMContentLoaded",o,{once:!0}):o()}catch(e){console.warn("[main.js] toc_actions falló",e)}})();

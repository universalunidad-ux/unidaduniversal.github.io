...
TRY("calc_header_logo",()=>{const D=document,W=window,getSys=()=>((D.getElementById("app")?.dataset.system)||"").trim(),findLogo=s=>((W.CATALOG_SISTEMAS||[]).find(x=>x.name===s)||{}).imgLogo||"",mount=()=>{const sys=getSys();if(!sys)return;const p=D.getElementById("calc-primary");if(!p||p.querySelector(".calc-head"))return;const logo=findLogo(sys),h=D.createElement("div");h.className="calc-head";h.innerHTML=`<div class="calc-head-row">${logo?`<img class="calc-syslogo" src="${logo}" alt="${sys}">`:""}<div class="calc-head-text"><div class="calc-price">Precio</div><div class="calc-sub">${sys}</div></div></div>`;p.prepend(h)};D.readyState==="loading"?D.addEventListener("DOMContentLoaded",mount,{once:!0}):mount();W.addEventListener("calc-render",mount,{passive:!0});W.addEventListener("pageshow",mount,{passive:!0});const p=D.getElementById("calc-primary");if(p&&!p._calcHeadObs){p._calcHeadObs=new MutationObserver(()=>mount());p._calcHeadObs.observe(p,{childList:!0,subtree:!0})}});

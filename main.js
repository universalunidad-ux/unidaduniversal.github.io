/* =========================================================
 ...
 - Carrusel universal .carousel + títulos:
   ...
   * VIDEOS: barra superior tipo “Videos del sistema” (solo títulos)
 - CarouselX sistemas + ListSlider + Calculadora + Iconos -15%
 - SIN duplicados (se eliminó bloque X) + FIX markReady
========================================================= */
(() => {
"use strict";
...
(() => {
  ...
  const pick=async(arr)=>{for(const u of arr) if(await exists(u)) return u; return arr[0];};
  ...
  const normalize=(root=D)=>{
    ...
    root.querySelectorAll(".js-abs-href[data-href]").forEach(a=>{
      ...
      const parts=p.split("#"); const pth=parts[0]||""; const hash=parts[1]||"";
...
(() => {
  ...
  const b=D.getElementById("burger"), m=D.getElementById("mobileMenu");
...
========================================================= */
(() => {
  if(W.__EXP_YT_MGR__) return; W.__EXP_YT_MGR__=1;
  ...
  /* FIX: markReady seguro (sin bug) */
  const markReady=(wrap)=>{
  ...
  const mountLazy=(wrap)=>{
    ...
    /* Si YA hay iframe en HTML, NO ponemos overlay */
    ...
    const load=()=>{
      ...
      /* CLAVE: activa modo iframe desde ya */
      wrap.classList.add("has-iframe");
...
/* =========================================================
 ...
 - Videos: alterna barra superior (solo títulos)
========================================================= */
(() => {
  ...
  const syncDots=(root,len)=>{
    ...
    while(dots.length>len){ const x=dots.pop(); if(x) x.remove(); }
    return dots;
  };
  ...
  /* ===== REELS: busca .reel-title asociados ===== */
  const titlesFor=(car)=>{
  ...
  /* ===== VIDEOS: barra superior tipo “Videos del sistema” ===== */
  const ensureVideosBar=(car)=>{
    if(!car || car.dataset.vbarInit==="1") return;
    car.dataset.vbarInit="1";

    const track=car.querySelector(".carousel-track");
    if(!track) return;

    const bar=D.createElement("div");
    bar.className="yt-titlesbar"; /* usa tu CSS */
    bar.setAttribute("role","presentation");

    const left=D.createElement("div");
    const right=D.createElement("div");
    left.className="yt-tab"; right.className="yt-tab";
    bar.appendChild(left); bar.appendChild(right);

    /* Inserta barra arriba del track */
    track.parentElement.insertBefore(bar,track);

    car._vbar={bar,left,right};
  };

  const slideVideoTitles=(slide)=>{
    if(!slide) return [];
    /* 1) h4.yt-title */
    const h=Array.from(slide.querySelectorAll(".yt-title")).map(x=>(x.textContent||"").trim()).filter(Boolean);
    if(h.length) return h;

    /* 2) data-title en wraps */
    const d=Array.from(slide.querySelectorAll(".yt-wrap[data-title],.reel-embed[data-title]"))
      .map(x=>(x.getAttribute("data-title")||"").trim()).filter(Boolean);
    if(d.length) return d;

    /* 3) iframe.title */
    const f=Array.from(slide.querySelectorAll("iframe[title]")).map(x=>(x.getAttribute("title")||"").trim()).filter(Boolean);
  ...
  const boot=()=>{
    D.querySelectorAll(".carousel").forEach(car=>{
      ...
      /* Asegura barra inicial en videos */
...
(() => {
  D.querySelectorAll(".listSlider").forEach(w=>{
    ...
    let i=0; const len=t.children.length||1;
    const go=(x)=>{
...
(() => {
  const ensureUI=(root)=>{
    ...
    if(!dotsWrap){ dotsWrap=D.createElement("div"); dotsWrap.className="group-dots"; root.appendChild(dotsWrap); }
  ...
  D.querySelectorAll(".carouselX").forEach(root=>{
    ...
    items.forEach(it=>{
      ...
      it.addEventListener("click",(e)=>{
        e.preventDefault();
        if(isMob() && !touched){
          touched=true; it.classList.add("show-hover");
...
(() => {
  D.addEventListener("DOMContentLoaded",()=>{
    ...
    const refresh=()=>{
      const ex=setSel(); ex.add(PRIMARY);

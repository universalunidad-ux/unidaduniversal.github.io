...
TRY("calc_heads_owner",()=>{const D=document,W=window,$=s=>D.querySelector(s),txt=id=>{const el=D.getElementById(id);return el?(el.textContent||"").trim():""},logoOf=name=>{const c=W.CATALOG_SISTEMAS||[];const f=c.find(x=>x&&x.name===name);return (f&&f.imgLogo)||""},head=(el)=>{if(!el)return null;let h=el.querySelector(".calc-head");if(h)return h;h=D.createElement("div");h.className="calc-head";h.innerHTML='<div class="calc-head-row"><img class="calc-syslogo" alt="" decoding="async"><div class="calc-head-text"><div class="calc-price"></div><div class="calc-sub"></div></div></div>';el.insertBefore(h,el.firstChild);return h},set=(el,{name,priceLabel,priceValue})=>{const h=head(el);if(!h)return;const img=h.querySelector(".calc-syslogo"),p=h.querySelector(".calc-price"),s=h.querySelector(".calc-sub");const src=logoOf(name);if(src){img.src=src;img.alt=name||"";img.style.display=""}else{img.style.display="none"}p.textContent=priceValue||priceLabel||"";s.textContent=name||""},setSlot=(el)=>{if(!el)return;let h=el.querySelector(".calc-head");if(!h){h=D.createElement("div");h.className="calc-head";h.innerHTML='<div class="calc-head-row"><img class="calc-syslogo" alt="" decoding="async"><div class="calc-head-text"><div class="calc-price"></div><div class="calc-sub"></div></div></div>';el.insertBefore(h,el.firstChild)}const img=h.querySelector(".calc-syslogo"),p=h.querySelector(".calc-price"),s=h.querySelector(".calc-sub");img.style.display="none";p.textContent="";s.textContent="Integra otro sistema y recibe −15%";},run=()=>{const p=$("#calc-primary"),s=$("#calc-secondary"),t=$("#calc-tertiary"),slot=$("#calc-slot-2");if(p){const name=p.dataset.systemName||"";set(p,{name,priceLabel:"Precio",priceValue:txt("tot1")})}if(s&&s.querySelector("table")){const name=s.dataset.systemName||"";set(s,{name,priceLabel:"Precio",priceValue:txt("tot2")})}if(t&&t.style.display!=="none"&&t.querySelector("table")){const name=t.dataset.systemName||"";set(t,{name,priceLabel:"Precio",priceValue:txt("tot3")})}slot&&setSlot(slot)};D.readyState==="loading"?D.addEventListener("DOMContentLoaded",run,{once:!0}):run();W.addEventListener("calc-render",run,{passive:!0});W.addEventListener("calc-recompute",run,{passive:!0});W.addEventListener("pageshow",run,{passive:!0});const row=D.getElementById("calc-row");if(row&&!row._headsObs){row._headsObs=new MutationObserver(()=>run());row._headsObs.observe(row,{childList:!0,subtree:!0})}}); 

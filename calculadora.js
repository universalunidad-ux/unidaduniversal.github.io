...
function cleanupSuffixDupes(suf,keep){var ids=["lic","op","rfc","usr","instOn","instdiscOn","base","uadd","disc","inst","sub","iva","tot","tr-base","tr-uadd","tr-disc","tr-inst","tr-sub","tr-iva","tr-tot","lbl-base","lbl-uadd","lbl-disc","lbl-inst","lbl-sub","lbl-iva","lbl-tot","instLblSTRONG","instLblTail"],seen=0;ids.forEach(p=>{var full=p+suf,nodes=D.querySelectorAll("#"+CSS.escape(full));if(nodes.length>1)nodes.forEach(n=>{if(keep&&keep.contains(n))return;var box=n.closest(".calc-container");box?(box.innerHTML="",box.removeAttribute("data-system-name")):n.remove()});nodes.length&&(seen+=nodes.length)});return seen}
...
var instWrap=D.createElement("div");instWrap.className="inst-wrap";instWrap.innerHTML='<div class="instalacion-box"><label><input type="checkbox" id="instOn'+idSuffix+'" checked><span><strong id="instLblSTRONG'+idSuffix+'">Instalación (1)</strong><span id="instLblTail'+idSuffix+'">: En tu equipo.</span></span></label></div>';form.appendChild(instWrap);container.appendChild(form);
...
var discountAmt=round2(subtotalSistemas*discountPct),afterDiscount=round2(subtotalSistemas-discountAmt),instGross=calcInstallationGross(),instDiscount=round2(instGross*0.5),instNet=round2(instGross-instDiscount);
 var instOn=instCheckbox(),instCount=instOn&&instOn.checked?usuarios:0,instWord=instCount===1?"Instalación":"Instalaciones",instLabel=(instCount>0?instWord+" ("+instCount+")":"Instalación")+" • -50% si es primer servicio",subLabel="Subtotal";var elLblInst=$id("lbl-inst"+idSuffix);elLblInst&&(elLblInst.textContent=instLabel);var elLblSub=$id("lbl-sub"+idSuffix);elLblSub&&(elLblSub.textContent=subLabel);

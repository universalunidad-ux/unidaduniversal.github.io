...
(()=>{var lock=0,txt=id=>{var e=$id(id);return e?(e.textContent||"").trim():""},logoOf=n=>{for(var c=W.CATALOG_SISTEMAS||[],i=0;i<c.length;i++)if(c[i]&&c[i].name===n)return c[i].imgLogo||"";return""},setT=(el,v)=>{v=v||"";el&&el.textContent!==v&&(el.textContent=v)},head=el=>{if(!el)return null;var h=el.querySelector(".calc-head");if(h)return h;h=D.createElement("div");h.className="calc-head";h.innerHTML='<div class="calc-head-row" style="display:flex;justify-content:center;align-items:center"><img class="calc-syslogo" alt="" decoding="async"></div>';el.insertBefore(h,el.firstChild);return h},setCalcHead=(el,name)=>{var h=head(el);if(!h)return;var img=h.querySelector(".calc-syslogo"),src=logoOf(name)||"";if(src){img.src=src;img.alt=name||"";img.style.cssText="height:68px;width:auto;max-width:260px;display:block;margin:0 auto"}else img.style.display="none"},ensureNote=(slot,title,sub)=>{if(!slot)return;var note=slot.querySelector(".note");note||(note=D.createElement("div"),note.className="note",slot.prepend(note));var st=note.querySelector("strong");st||(st=D.createElement("strong"),note.appendChild(st));var sm=note.querySelector("small");sm||(sm=D.createElement("small"),note.appendChild(sm));setT(st,title||"");sm.innerHTML=sub?("<em>"+sub+"</em>"):""},setSlot2=el=>{el&&ensureNote(el,"Integra otro sistema y recibe −15%","XML en Línea+ no aplica para descuento.")},setAddMore=el=>{el&&ensureNote(el,"¿Agregar un tercer sistema?","Mismo -15% por paquete, excepto XML en Línea+.")},brandRG=()=>{var r=$id("calculadora-section");if(!r)return;for(var nodes=r.querySelectorAll(".calc-sub,.sys-icon strong"),i=0;i<nodes.length;i++){var el=nodes[i];if(!el||el.dataset.calcRg==="1")continue;var t=el.textContent||"";if(!/\bCONTPAQi\b/.test(t))continue;/\bCONTPAQi®\b/.test(t)?el.dataset.calcRg="1":(el.textContent=t.replace(/\bCONTPAQi\b/g,"CONTPAQi\u00AE"),el.dataset.calcRg="1")}},run=()=>{if(lock)return;lock=1;try{var p=$id("calc-primary"),s=$id("calc-secondary"),t=$id("calc-tertiary"),slot=$id("calc-slot-2"),add=$id("add-more-panel");p&&setCalcHead(p,p.dataset.systemName||"");s&&s.querySelector("table")&&setCalcHead(s,s.dataset.systemName||"");t&&t.style.display!=="none"&&t.querySelector("table")&&setCalcHead(t,t.dataset.systemName||"");slot&&setSlot2(slot);add&&setAddMore(add);brandRG()}finally{lock=0}},rq=0,runQ=()=>{rq||(rq=requestAnimationFrame(()=>{rq=0;run()}))};D.readyState==="loading"?D.addEventListener("DOMContentLoaded",runQ,{once:!0}):runQ();W.addEventListener("calc-render",runQ,{passive:!0});W.addEventListener("pageshow",runQ,{passive:!0});var row=$id("calc-row");row&&!row._headsObs&&(row._headsObs=new MutationObserver(()=>runQ()),row._headsObs.observe(row,{childList:!0,subtree:!1}))})();

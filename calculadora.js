...
head=el=>{if(!el)return null;var h=el.querySelector(".calc-head");if(h)return h;h=D.createElement("div");h.className="calc-head";h.innerHTML='<div class="calc-head-row" style="display:flex;justify-content:center;align-items:center"><img class="calc-syslogo" alt="" decoding="async"></div>';el.insertBefore(h,el.firstChild);return h}
setCalcHead=(el,name)=>{var h=head(el);if(!h)return;var img=h.querySelector(".calc-syslogo"),src=logoOf(name);if(src){img.style.display="";setA(img,"src",src);setA(img,"alt",name||"");img.style.height="68px";img.style.width="auto";img.style.maxWidth="260px"}else img.style.display="none"}
...
run=()=>{if(lock)return;lock=1;try{var p=$id("calc-primary"),s=$id("calc-secondary"),t=$id("calc-tertiary"),slot=$id("calc-slot-2"),add=$id("add-more-panel");p&&setCalcHead(p,p.dataset.systemName||"");s&&s.querySelector("table")&&setCalcHead(s,s.dataset.systemName||"");t&&t.style.display!=="none"&&t.querySelector("table")&&setCalcHead(t,t.dataset.systemName||"");slot&&setSlot2(slot);add&&setAddMore(add);brandRG()}finally{lock=0}},rq=0,runQ=()=>{rq||(rq=requestAnimationFrame(()=>{rq=0;run()}))};

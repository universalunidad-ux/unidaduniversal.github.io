...
function paintSystemButtons(wrapId,onPick,exclude){var wrap=$id(wrapId);if(!wrap)return;wrap.innerHTML="";var list=getSystemList();exclude=exclude||[];list.forEach(item=>{if(!item||!item.name)return;if(exclude.indexOf(item.name)!==-1)return;var b=D.createElement("button");b.type="button";b.className="sys-icon";b.setAttribute("data-system",item.name);b.setAttribute("data-sys",item.name);b.dataset.system=item.name;b.dataset.sys=item.name;b.innerHTML=item.img?'<img src="'+item.img+'" alt=""><strong>'+item.name+"</strong>":"<strong>"+item.name+"</strong>";b.addEventListener("click",()=>onPick(item.name));wrap.appendChild(b)})}
...
e1&&(filas.push({label:"Subtotal "+n1+" e "+instLabelFor("1"),val:getNum("sub1")}),totales.push(getNum("tot1")),ivaTotal+=getNum("iva1"));
...
/* HEADERS OWNER (safe-min + textos solicitados) */
(()=>{var lock=0,txt=id=>{var e=$id(id);return e?(e.textContent||"").trim():""},logoOf=n=>{var c=W.CATALOG_SISTEMAS||[],i=0;for(i=0;i<c.length;i++)if(c[i]&&c[i].name===n)return c[i].imgLogo||"";return""},setT=(el,v)=>{v=v||"";el&&el.textContent!==v&&(el.textContent=v)},setA=(el,a,v)=>{v=v||"";el&&el.getAttribute(a)!==v&&el.setAttribute(a,v)},
head=el=>{if(!el)return null;var h=el.querySelector(".calc-head");if(h)return h;h=D.createElement("div");h.className="calc-head";h.innerHTML='<div class="calc-head-row"><img class="calc-syslogo" alt="" decoding="async"><div class="calc-head-text"><div class="calc-price"></div><div class="calc-sub"></div></div></div>';el.insertBefore(h,el.firstChild);return h},
setCalcHead=(el,name,price)=>{var h=head(el);if(!h)return;var img=h.querySelector(".calc-syslogo"),p=h.querySelector(".calc-price"),s=h.querySelector(".calc-sub"),src=logoOf(name);if(src){img.style.display!==""&&(img.style.display="");setA(img,"src",src);setA(img,"alt",name||"")}else img.style.display!=="none"&&(img.style.display="none");setT(p,price||"");setT(s,name||"")},
ensureNote=(slot,title,sub)=>{if(!slot)return;var note=slot.querySelector(".note");if(!note){note=D.createElement("div");note.className="note";slot.prepend(note)}var st=note.querySelector("strong");st||(st=D.createElement("strong"),note.appendChild(st));var sm=note.querySelector("small");sm||(sm=D.createElement("small"),note.appendChild(sm));setT(st,title||"");sm.innerHTML=sub?("<em>"+sub+"</em>"):""},
setSlot2=el=>{if(!el)return;ensureNote(el,"Integra otro sistema y recibe −15%","XML en Línea+ no aplica para descuento.");},
setAddMore=el=>{if(!el)return;ensureNote(el,"¿Agregar un tercer sistema?","Mismo -15% por paquete, excepto XML en Línea+.");},
brandRG=()=>{var r=$id("calculadora-section");if(!r)return;var nodes=r.querySelectorAll(".calc-sub,.sys-icon strong"),i=0;for(i=0;i<nodes.length;i++){var el=nodes[i];if(!el||el.dataset.calcRg==="1")continue;var t=el.textContent||"";if(!/\bCONTPAQi\b/.test(t))continue;if(/\bCONTPAQi®\b/.test(t)){el.dataset.calcRg="1";continue}el.textContent=t.replace(/\bCONTPAQi\b/g,"CONTPAQi\u00AE");el.dataset.calcRg="1"}},
run=()=>{if(lock)return;lock=1;try{var p=$id("calc-primary"),s=$id("calc-secondary"),t=$id("calc-tertiary"),slot=$id("calc-slot-2"),add=$id("add-more-panel");p&&setCalcHead(p,p.dataset.systemName||"",txt("tot1"));s&&s.querySelector("table")&&setCalcHead(s,s.dataset.systemName||"",txt("tot2"));t&&t.style.display!=="none"&&t.querySelector("table")&&setCalcHead(t,t.dataset.systemName||"",txt("tot3"));slot&&setSlot2(slot);add&&setAddMore(add);brandRG()}finally{lock=0}},rq=0,runQ=()=>{rq||(rq=requestAnimationFrame(()=>{rq=0;run()}))};
D.readyState==="loading"?D.addEventListener("DOMContentLoaded",runQ,{once:!0}):runQ();
W.addEventListener("calc-render",runQ,{passive:!0});
W.addEventListener("pageshow",runQ,{passive:!0});
var row=$id("calc-row");
row&&!row._headsObs&&(row._headsObs=new MutationObserver(()=>runQ()),row._headsObs.observe(row,{childList:!0,subtree:!1}));

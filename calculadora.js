...
/*! Expiriti tooltips v1.1 */(()=>{const d=document,w=window,SEL=".ux-info,.ux-tip",C="exp-tip";let tip=null,openEl=null;const kill=()=>{tip&&(tip.remove(),tip=null),openEl&&openEl.setAttribute("aria-expanded","false"),openEl=null},esc=e=>{(e.key==="Escape"||e.key==="Esc")&&kill()},outside=e=>{tip&&!tip.contains(e.target)&&openEl&&!openEl.contains(e.target)&&kill()},repos=()=>{if(!tip||!openEl)return;const r=openEl.getBoundingClientRect(),tr=tip.getBoundingClientRect();let x=r.left+r.width/2-tr.width/2,y=r.top-tr.height-10;const pad=10;x=Math.max(pad,Math.min(x,w.innerWidth-tr.width-pad)),y<pad&&(y=r.bottom+10),tip.style.left=x+"px",tip.style.top=y+"px"};const textOf=el=>el.getAttribute("data-tip")||el.getAttribute("title")||"";const arm=el=>{if(!el.getAttribute("data-tip")){const t=el.getAttribute("title")||"";t&&el.setAttribute("data-tip",t)}el.setAttribute("title",""),el.setAttribute("role","button"),el.setAttribute("tabindex","0")};const show=el=>{if(openEl===el){kill();return}kill();arm(el);const t=textOf(el);if(!t)return;el.setAttribute("aria-expanded","true"),openEl=el,tip=d.createElement("div"),tip.className=C,tip.textContent=t,d.body.appendChild(tip),repos()};const on=e=>{const el=e.target.closest?.(SEL);if(!el)return;e.preventDefault(),e.stopPropagation(),show(el)};d.addEventListener("pointerdown",on,!0);d.addEventListener("click",on,!0);d.addEventListener("keydown",e=>{const el=e.target.closest?.(SEL);if(!el)return;(e.key==="Enter"||e.key===" ")&&(e.preventDefault(),e.stopPropagation(),show(el))},!0);d.addEventListener("keydown",esc);d.addEventListener("pointerdown",outside);w.addEventListener("scroll",kill,{passive:!0});w.addEventListener("resize",kill,{passive:!0});w.addEventListener("calc-render",()=>kill(),{passive:!0})})();

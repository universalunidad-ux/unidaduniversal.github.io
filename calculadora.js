...
if(document?.body?.getAttribute("data-calc")==="nube"||window.__EXPIRITI_FORCE_NUBE__===true){
  console.log("calculadora.js v13.1: saltado COMPLETO (modo Nube activo)");
  window.CalculadoraContpaqi=window.CalculadoraContpaqi||{init(){},setSecondarySystem(){},setTertiarySystem(){},updateCombinedSummary(){}};
}else{

/* calculadora.js v13.1 — FIX Subtotal+IVA (incluye instalación) + Total bold real + labels */
(function(){
"use strict";
console.log("calculadora.js v13.1 cargado — MultiRFC default + Operación solo en Tradicional");

  // ========================= Helpers =========================
  var money=new Intl.NumberFormat("es-MX",{style:"currency",currency:"MXN",maximumFractionDigits:0});
  function fmt(v){return money.format(Math.round(Number(v||0)))} /* MXN redondeado */
  ...
  function createCalculator(container,sistemaName,idSuffix,combinedSelector){
    ...
    opLabel.className="op-label"; /* se muestra con JS según Licencia */
    ...
    table.innerHTML=
      ...
      /* PATCH: label correcto */
      '  <tr id="tr-sub'+idSuffix+'"><td>Subtotal (sistemas + instalación)</td><td id="sub'+idSuffix+'">$0</td></tr>'+
    ...
    function calculateAndRender(){
      ...
      /* ===== PATCH v13.1: Subtotal incluye instalación + Total bold real ===== */
      ...
      /* PATCH: Subtotal ahora = sistemas(desc) + instalación(neta) */
      document.getElementById("sub"+idSuffix).textContent=fmt(baseImponible);
      /* IVA ya incluye sistemas + instalación (baseImponible) */
      document.getElementById("iva"+idSuffix).textContent=fmt(iva);
      /* PATCH: Total en bold (no textContent, si no borra <strong>) */
    ...
    function writeZeros(){
      ...
      /* PATCH: mantener bold aunque sea cero */
  ...
  function updateCombinedSummary(combinedSelector){
    ...
    if(!e2&&!e3){
      /* PATCH: deja tu resumen simple cuando solo hay 1 sistema */
    ...
    /* PATCH: label alineado con tu nuevo sub = sistemas + instalación */
  ...
  function autoInit(){
    if(window.__EXPIRITI_FORCE_NUBE__===true||document.body.getAttribute("data-calc")==="nube"){
      console.log("v13.1: cancelado autoInit porque hay calculadora NUBE");
      return;
    }

...
if(document?.body?.getAttribute("data-calc")==="nube"||window.__EXPIRITI_FORCE_NUBE__===true){
  ...
  console.log("calculadora.js v13.2a (FIX HTML real): cargado");
...
function ensureSecondaryContainer(){
  ...
  function showThirdPanel(show){
    ...
    var p=$id("add-more-panel");
    if(t) t.style.display=show?"":"none";
    if(p) p.style.display=show?"none":"";
  }

  // ===== Render iconos para seleccionar sistemas =====
  function getSystemList(){
    // Si ya tienes CATALOG_SISTEMAS, úsalo. Si no, genera desde preciosContpaqi.
  ...
  // ===== Parche CSS mínimo (opcional y seguro) =====
  (function(){
  ...
  // ===================== Render calculadora (TU LÓGICA COMPLETA) =====================
  function createCalculator(container,sistemaName,idSuffix,combinedSelector){
    ...
    function refreshOptions(){
      ...
      var multi=Array.from(rfcSel.options).find(o=>/multirfc/i.test(o.text));
    ...
    function calculateAndRender(){
      ...
      // -15% si hay paquete (2 o 3) y NO es XML
      var discountPct=0;
      ...
      var elLblDisc=$id("lbl-disc"+idSuffix); if(elLblDisc) elLblDisc.textContent="Descuento (sistema)";
    ...
    opSel.addEventListener("change",function(){
      ...
      if(lic==="tradicional"&&op==="crecimiento_usuario"){rfcLabel.style.display="none"}
        ...
        if(rfcSel.options.length===0){
          ...
          var m=Array.from(rfcSel.options).find(o=>/multirfc/i.test(o.text)); if(m) rfcSel.value=m.value;
        }
  ...
  // =================== Resumen combinado (usa TU tabla #combined-table-body) =====================
  function updateCombinedSummary(combinedSelector){
    ...
    var n1=($id("calc-primary")?.dataset?.systemName)||"Sistema 1";
    var n2=($id("calc-secondary")?.dataset?.systemName)||"Sistema 2";
    var n3=($id("calc-tertiary")?.dataset?.systemName)||"Sistema 3";

    function instCountFor(suf){
      var chk=$id("instOn"+suf);
      if(!chk||!chk.checked) return 0;
      var u=$id("usr"+suf);
      return Math.max(1,parseInt((u&&u.value)||"1",10)||1);
    }
    ...
    var totalCombinado=round2(totales.reduce((a,b)=>a+b,0));
  ...
  // ====================== API pública (ajustada a TU HTML) =======================
  function initCalculadora(opts){
    ...
    // pinta iconos para seleccionar 2º sistema
  ...
  function setSecondarySystem(name,opts){
    ...
    if(!sec){console.warn("No se pudo montar #calc-secondary en #calc-slot-2");return}
    createCalculator(sec,name,"2",combinedSelector);

    // cuando ya hay 2, habilita panel de 3er sistema
    showThirdPanel(false);
    var exclude=[($id("calc-primary")?.dataset?.systemName)||"",name].filter(Boolean);
    ...
    // muestra el panel “agregar 3er sistema”
    showThirdPanel(false);
  ...
  function setTertiarySystem(name,opts){
    ...
    // ya con 3, oculta panel add-more
    showThirdPanel(true);
  ...
  // Auto-init
  function autoInit(){
    if(window.__EXPIRITI_FORCE_NUBE__===true||document.body.getAttribute("data-calc")==="nube"){console.log("cancelado autoInit: modo nube");return}

...
if(document.readyState==="loading") document.addEventListener("DOMContentLoaded",autoInitOnce,{once:true});
                  ...
                  /* HEADERS OWNER (min) */(()=>{var D=document,W=window,txt=id=>{var e=D.getElementById(id);return e?(e.textContent||"").trim():""},logoOf=n=>{var c=W.CATALOG_SISTEMAS||[],f=null,i=0;for(i=0;i<c.length;i++){if(c[i]&&c[i].name===n){f=c[i];break}}return(f&&f.imgLogo)||""},head=el=>{if(!el)return null;var h=el.querySelector(".calc-head");if(h)return h;h=D.createElement("div");h.className="calc-head";h.innerHTML='<div class="calc-head-row"><img class="calc-syslogo" alt="" decoding="async"><div class="calc-head-text"><div class="calc-price"></div><div class="calc-sub"></div></div></div>';el.insertBefore(h,el.firstChild);return h},set=(el,name,price)=>{var h=head(el);if(!h)return;var img=h.querySelector(".calc-syslogo"),p=h.querySelector(".calc-price"),s=h.querySelector(".calc-sub"),src=logoOf(name);if(src){img.src=src;img.alt=name||"";img.style.display=""}else img.style.display="none";p.textContent=price||"Precio";s.textContent=name||""},setSlot=el=>{if(!el)return;var h=head(el);if(!h)return;var img=h.querySelector(".calc-syslogo"),p=h.querySelector(".calc-price"),s=h.querySelector(".calc-sub");img.style.display="none";p.textContent="";s.textContent="Integra otro sistema y recibe −15%"},brandRG=()=>{var r=D.getElementById("calculadora-section");if(!r)return;var n=r.querySelectorAll(".calc-sub,.sys-icon strong"),i=0;for(i=0;i<n.length;i++){var el=n[i];if(!el||el.dataset.calcRg==="1")continue;var t=(el.textContent||"");if(!/\bCONTPAQi\b/.test(t))continue;el.textContent=t.replace(/\bCONTPAQi\b/g,"CONTPAQi\u00AE");el.dataset.calcRg="1"}},run=()=>{var p=D.getElementById("calc-primary"),s=D.getElementById("calc-secondary"),t=D.getElementById("calc-tertiary"),slot=D.getElementById("calc-slot-2");p&&set(p,p.dataset.systemName||"",txt("tot1"));s&&s.querySelector("table")&&set(s,s.dataset.systemName||"",txt("tot2"));t&&t.style.display!=="none"&&t.querySelector("table")&&set(t,t.dataset.systemName||"",txt("tot3"));slot&&setSlot(slot);brandRG()};D.readyState==="loading"?D.addEventListener("DOMContentLoaded",run,{once:!0}):run();W.addEventListener("calc-render",run,{passive:!0});W.addEventListener("calc-recompute",run,{passive:!0});W.addEventListener("pageshow",run,{passive:!0});var row=D.getElementById("calc-row");row&&!row._headsObs&&(row._headsObs=new MutationObserver(()=>run()),row._headsObs.observe(row,{childList:!0,subtree:!0}))})();

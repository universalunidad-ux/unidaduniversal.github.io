...
function cleanupSuffixDupes(suf,keep){var ids=["lic","op","rfc","usr","instOn","base","uadd","disc","inst","sub","iva","tot","tr-base","tr-uadd","tr-disc","tr-inst","tr-sub","tr-iva","tr-tot","lbl-base","lbl-uadd","lbl-disc","lbl-inst","lbl-sub","lbl-iva","lbl-tot","instLblSTRONG"],seen=0;ids.forEach(p=>{var full=p+suf,nodes=D.querySelectorAll("#"+CSS.escape(full));if(nodes.length>1)nodes.forEach(n=>{if(keep&&keep.contains(n))return;var box=n.closest(".calc-container");box?(box.innerHTML="",box.removeAttribute("data-system-name")):n.remove()});nodes.length&&(seen+=nodes.length)});return seen}
...
var discountAmt=round2(subtotalSistemas*discountPct),afterDiscount=round2(subtotalSistemas-discountAmt),instGross=calcInstallationGross(),instDiscOnEl=$id("instdiscOn"+idSuffix),instDiscEnabled=!!(instDiscOnEl&&instDiscOnEl.checked),instDiscount=round2(instGross*(instDiscEnabled?0.5:0)),instNet=round2(instGross-instDiscount);
...
var strong=$id("instLblSTRONG"+idSuffix),tailEl=$id("instLblTail"+idSuffix),nInst=Math.max(1,usuarios);strong&&(strong.textContent="Instalación ("+nInst+")");tailEl&&(tailEl.textContent=nInst===1?": En tu equipo.":": En el equipo de cada usuario.");

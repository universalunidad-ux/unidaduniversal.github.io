...
function ensureCalcHead(e){if(!e)return null;let h=e.querySelector(".calc-head");if(h)return h;h=document.createElement("div");h.className="calc-head";h.innerHTML='<div class="calc-head-row"><img class="calc-syslogo" alt="" decoding="async"><div><div class="calc-price"></div><div class="calc-sub"></div></div></div>';e.insertBefore(h,e.firstChild);return h}

function setCalcHead(e,{logoSrc:l,title:t,subtitle:s,priceText:p}){const h=ensureCalcHead(e);if(!h)return;const i=h.querySelector(".calc-syslogo"),r=h.querySelector(".calc-price"),u=h.querySelector(".calc-sub");i.src=l||"";i.alt=t||"";r.textContent=p||"";u.textContent=s||t||""}

TRY("calc_heads",()=>{const A=()=>{const p=document.querySelector("#calc-primary"),s=document.querySelector("#calc-secondary"),t=document.querySelector("#calc-tertiary");p&&setCalcHead(p,{logoSrc:"IMG/comercial-premium.webp",title:"CONTPAQi Comercial PREMIUM",subtitle:"Precio",priceText:document.querySelector("#tot-primary")?.textContent||""});s&&setCalcHead(s,{logoSrc:"IMG/contabilidad.webp",title:"CONTPAQi Contabilidad",subtitle:"Precio",priceText:document.querySelector("#tot-secondary")?.textContent||""});t&&setCalcHead(t,{logoSrc:"IMG/bancos.webp",title:"CONTPAQi Bancos",subtitle:"Precio",priceText:document.querySelector("#tot-tertiary")?.textContent||""})};document.readyState==="loading"?document.addEventListener("DOMContentLoaded",A,{once:!0}):A();addEventListener("calc-render",A,{passive:!0});addEventListener("calc-recompute",A,{passive:!0});addEventListener("pageshow",A,{passive:!0})});

...
// Atajos para selección de nodos
const $$ = (sel, ctx = document) => Array.from(ctx.querySelectorAll(sel));
const $  = (sel, ctx = document) => ctx.querySelector(sel);

// ======================================================
// Carga de parciales (HEAD / HEADER / FOOTER)
// ======================================================

function getBasePath() {
  const path = window.location.pathname;
  // Páginas en subcarpetas (SISTEMAS, SERVICIOS, etc.)
  if (path.includes('/SISTEMAS/') || path.includes('/SERVICIOS/') || path.includes('/PDFS/') || path.includes('/CSS/')) {
    return '../';
  }
  // Páginas en raíz (index.html, pruebas, etc.)
  return './';
}
...
$$('.card.product-card[data-href]').forEach(card => {
  ...
  card.addEventListener('click', (e) => {
    if (e.target.closest('a')) return; // deja funcionar links internos
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  // Miniaturas simples de la sección "Videos"
  initSimpleThumbs();

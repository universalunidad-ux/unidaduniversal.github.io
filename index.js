...
function initHeaderLogic(){
  ...
  $$('.js-abs-src[data-src]').forEach(img=>{
    ...
    let finalSrc=raw;
    if(base==="../"&&!raw.startsWith("http")) finalSrc=base+raw;
    img.src=finalSrc;
...
function buildHeroGallerySlides(groupKey,sysKey){
  ...
  sys.images.forEach((item,idx)=>{
    ...
    img.height=550;
    img.decoding="async";
    const isLCP=groupKey===defaultGroup&&sysKey===configGroup.defaultSys&&idx===0;
    if(isLCP){
      img.loading="eager";
...
function buildHeroSystemTabs(groupKey){
  ...
  Object.entries(configGroup.systems).forEach(([sysKey,sys])=>{
    ...
    btn.innerHTML=`<img src="${sys.icon}" alt="${sys.label}" width="56" height="56" loading="lazy" decoding="async"><span>${sys.label}</span>`;
...
function renderReelIframe(wrap){
  ...
  const title=wrap.dataset.title||"";
  if(!id)return;
  wrap.innerHTML=`<iframe src="https://www.youtube-nocookie.com/embed/${id}?autoplay=1" title="${title}" loading="lazy" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen referrerpolicy="strict-origin-when-cross-origin"></iframe>`;
}

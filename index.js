/* =========================================================
   Expiriti - index.js (FINAL V6 MIN+FIX+WHEEL+HARDENED) — PATCHED (FULL)
   - SIN conflicto de $/$$
   ...
   - HERO Gallery: grupos + tabs + dots + arrows + scroll sync (FIX offsetLeft + sync robusto por offsets)
   - REELS: título 1-línea (FIX determinístico) + flechas off si 1 + scroll/trackpad sync (FIX offsetLeft + sync robusto por offsets)
   - Servicios: pager (mobile) + dots (FIX re-init safe + generalizado N páginas)
...
const DEBUG_NOCACHE=/[?&]nocache=1\b/.test(location.search);

/* =========================
   0.1) Preload imagen (para LCP cuando aplique)
========================= */
function addPreloadImage(href){if(!href)return;const abs=prefix(href);
  if(document.querySelector(`link[rel="preload"][as="image"][href="${abs}"]`))return;
  const l=document.createElement("link");l.rel="preload";l.as="image";l.href=abs;document.head.appendChild(l);
}

/* =========================
   1) Rutas (GH Pages + local) — FIX profundidad real
   - depth real por # de segmentos (más robusto)
========================= */
...
/* =========================
   5) Tabs Productos (sin duplicar) + autoscroll del tab activo si overflow
========================= */
function scrollTabIntoView(btn){
  ...
  const targetLeft=wrap.scrollLeft+(bRect.left-wRect.left)-(wRect.width/2-bRect.width/2);
...
function initPromosFilter(){
  const promoBtns=QA(".promo-btn"),promoItems=QA("#promoGrid [data-type]");
...
/* =========================
   7.1) Helper: índice más cercano por offsets (robusto con gap/padding/snap)
========================= */
function closestIndexByOffset(track,slides){
  const x=(track.scrollLeft||0)+(track.clientWidth/2);
  let best=0,bestDist=Infinity;
  for(let i=0;i<slides.length;i++){
    const mid=slides[i].offsetLeft+(slides[i].clientWidth/2);
...
function buildHeroGallerySlides(groupKey,sysKey){
  ...
  sys.images.forEach((item,idx)=>{
    ...
    if(isLCP){img.loading="eager";img.setAttribute("fetchpriority","high");addPreloadImage(item.src);}else img.loading="lazy";
...
function initHeroGallery(){
  ...
  const getIdxFromScroll=()=>{
    ...
    return closestIndexByOffset(track,slides);
  };
...
/* FIX: Título reels determinístico */
function setSingleLineReelTitle(cfg,title){
  const t=(title||"").trim(),el=cfg?.titleEl||null;if(!el)return;
  el.textContent=t;el.style.display="";el.removeAttribute("aria-hidden");
}
...
function renderReelThumb(wrap){
  ...
  if(btn&&btn.dataset.bound!=="1"){btn.dataset.bound="1";on(btn,"click",()=>{stopAllReels();renderReelIframe(wrap);});}
}
...
function initReelsCarousel(panelKey){
  ...
  if(cfg.carousel.dataset.scrollSync!=="1"){
    ...
    const syncFromScroll=()=>{
      ...
      /* ✅ FIX: índice por offsets (robusto con gap/padding/snap) */
      const idx=closestIndexByOffset(track,slides);
...
function initReelsTabs(){
  QA(".reel-tab").forEach(tab=>{
    ...
    on(tab,"click",()=>{
      ...
      stopAllReels();
      QA(".reel-tab").forEach(t=>{if(t.dataset.panel===panelKey)t.classList.toggle("active",t===tab);});
...
/* =========================
   ...
   - Listener scroll se bindea 1 sola vez
   - En cada init, actualiza root.__svcPagerSync para dots actuales
========================= */
function initServicesPager(){
  ...
  /* índice por offsets (centro del viewport) */
  ...
  if(root.dataset.pagerBound!=="1"){
    ...
    root.addEventListener("scroll",()=>{
      if(raf)cancelAnimationFrame(raf);
      raf=requestAnimationFrame(()=>{if(typeof root.__svcPagerSync==="function")root.__svcPagerSync();});
    },{passive:true});
  }
...
(function(){"use strict";
  function addPreconnect(href){
    ...
    l.rel="preconnect";l.href=href;l.crossOrigin="anonymous";document.head.appendChild(l);
  }
  ...
  if(document.readyState==="loading"){document.addEventListener("DOMContentLoaded",initLazyMap,{once:true});}
  else{initLazyMap();}
})();
